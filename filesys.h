...
// struct for returning a block index plus internal direntry index
typedef struct entrylc {
    int blocki;
    int entryi;
} direntryloc;
...
typedef char strblock_t [BLOCKSIZE/sizeof(char)];
...
typedef struct filedescriptor {
   ...
   int         tot ;           // total bytes read/written so far
   direntryloc loc ;           // remember where the directory entry is
   char        mode[3] ;
   Byte        writing ;
   fatentry_t  blockno ;       // block index of buffer contents
   diskblock_t buffer ;
} MyFILE ;
